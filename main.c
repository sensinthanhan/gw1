...
static int  ip6_send_cmd_hello (int nodeid, int port, int retrans, bool encryption_en);
static int  ip6_send_cmd_key (int nodeid, int port, int retrans, bool encryption_en);
...
static char sim_gw_ipv6[40] = "fd00::c30c:0:0:1"; 
#endif
...
void set_node_app_key (int node_id) {
    ...
    res = ip6_send_cmd_key(node_id, SLS_NORMAL_PORT, NUM_RETRANS_SET_KEY, node_db_list[node_id].encryption_phase);
    if (res == -1) {
...
void show_local_db() { 
    ...
    if (SHOW_FULL_DB == TRUE) {        
        ...
        for(i = 0; i < num_of_node; i++) {
            if (i>0) {
                printf("| %2d |\033[1;33m %24s \033[0m|\033[1;32m%2s/%1d\033[0m|%5d|%5d|%5d| 0x%02X|%5d| %17s |%5d|%4d/%5u/%3X|%4d(%4d)/%5.1f| 0x%04X | _%02x%02x|%7.02f|%5.1f|\033[1;35m%8.02f\033[0m|\033[1;35m%5d\033[0m|\033[1;35m%8.1f\033[0m|\033[1;35m%5.2f\033[0m|\n",node_db_list[i].id,
                ...
                printf("| %2d |\033[1;33m %24s \033[0m|\033[1;32m*%1s/%1d\033[0m|%5d|%5d|%5d| 0x%02X|%5d| %17s |%5d|%4d/%5u/%3X|%4d(%4d)/%5.1f| 0x%04X | _%02x%02x|%7.02f|%5.1f|\033[1;35m%8.02f\033[0m|\033[1;35m%5d\033[0m|\033[1;35m%8.1f\033[0m|\033[1;35m%5.2f\033[0m|\n",node_db_list[i].id,
                    node_db_list[i].ipv6_addr, node_db_list[i].connected,node_db_list[i].authenticated, node_db_list[i].num_req, 
                    node_db_list[i].num_rep, node_db_list[i].num_timeout, node_db_list[i].last_cmd, node_db_list[i].num_of_retrans,
                    node_db_list[i].last_seen, node_db_list[i].channel_id, node_db_list[i].rssi, node_db_list[i].lqi, node_db_list[i].tx_power, 
                    node_db_list[i].num_emergency_msg, node_db_list[i].async_seq, node_db_list[i].async_prr, node_db_list[i].last_err_code,
    ...
    // show short table     
        ...
        for(i = 0; i < num_of_node; i++) {
            if (i>0) {
                printf("| %2d |\033[1;33m %24s \033[0m|\033[1;32m%2s/%1d\033[0m|%5d|%5d|%5d| 0x%02X|%5d|%5d|%4d/%5u/%3X|%4d(%4d)/%5.1f| 0x%04X | _%02x%02x|%7.02f|%5.1f|\n",node_db_list[i].id,
                    node_db_list[i].ipv6_addr, node_db_list[i].connected,node_db_list[i].authenticated, node_db_list[i].num_req, 
                    node_db_list[i].num_rep, node_db_list[i].num_timeout, node_db_list[i].last_cmd, node_db_list[i].num_of_retrans,
                ...
                printf("| %2d |\033[1;33m %24s \033[0m|\033[1;32m*%1s/%1d\033[0m|%5d|%5d|%5d| 0x%02X|%5d|%5d|%4d/%5u/%3X|%4d(%4d)/%5.1f| 0x%04X | _%02x%02x|%7.02f|%5.1f|\n",node_db_list[i].id,
                    node_db_list[i].ipv6_addr, node_db_list[i].connected,node_db_list[i].authenticated, node_db_list[i].num_req, 
                    node_db_list[i].num_rep, node_db_list[i].num_timeout, node_db_list[i].last_cmd, node_db_list[i].num_of_retrans,
                    node_db_list[i].channel_id, node_db_list[i].rssi, node_db_list[i].lqi, node_db_list[i].tx_power, 
                    node_db_list[i].num_emergency_msg, node_db_list[i].async_seq, node_db_list[i].async_prr, node_db_list[i].last_err_code,
...
#ifdef SIMULATION_BUILD
        ptr_file =fopen("cooja_node_z1.txt","r");
#else
...
bool authenticate_node(int node_id, uint32_t challenge_code, uint16_t challenge_res, int *res) {
    ...
    response = ip6_send_cmd_hello(node_id, SLS_NORMAL_PORT, NUM_RETRANS_AUTHEN, node_db_list[node_id].encryption_phase);    
    if (response == -1) {
...
/* calculate CRC
    encrypt 64 byte of cmd
*/
void make_packet_for_node_hello(cmd_struct_t *cmd, uint16_t nodeid){

    static uint8_t key_begin[16] = "thitgahaplachanh";

    tx_cmd.crc = 0;
    gen_crc_for_cmd(cmd);
    encrypt_payload(cmd, key_begin);
    printf(" - Make Tx packet for node %d...done; Encryption:\033[0;31m ENABLED \033[0m \n", nodeid);        
}
...
/* calculate CRC
    encrypt 64 byte of cmd
*/
void make_packet_for_node_key(cmd_struct_t *cmd, uint16_t nodeid){
...
void check_packet_for_node_hello(cmd_struct_t *cmd, uint16_t nodeid){
    static uint8_t key_begin[16] = "thitgahaplachanh";
    decrypt_payload(cmd, key_begin);
...
void check_packet_for_node_key(cmd_struct_t *cmd, uint16_t nodeid){
    static uint8_t key_begin[16] = "thitgahaplachanh";
    decrypt_payload(cmd, key_begin);
...
static int ip6_send_cmd_hello(int nodeid, int port, int retrans, bool encryption_en) {
    ...
    while (num_of_retrans < retrans) {
        ...
        /* encrypt payload here */
        make_packet_for_node_hello(&tx_cmd, nodeid);
        //make_packet_for_node(&tx_cmd, nodeid, node_db_list[nodeid].authenticated);
        ...
        if (res == -1) {
            ...
            if (rev_bytes>=0) {
                ...
                check_packet_for_node_hello(&rx_reply, nodeid);    
...
static int ip6_send_cmd_key(int nodeid, int port, int retrans, bool encryption_en) {
    ...
    while (num_of_retrans < retrans) {
        ...
        /* encrypt payload here */
        make_packet_for_node_key(&tx_cmd, nodeid);
        //make_packet_for_node(&tx_cmd, nodeid, node_db_list[nodeid].authenticated);
        ...
        if (res == -1) {
            ...
            if (rev_bytes>=0) {
                ...
                check_packet_for_node_key(&rx_reply, nodeid);    
